                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <div className="flex flex-wrap gap-2">
                    <label className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 text-sm font-medium text-slate-700">
                      ...
                      <span>{imageUploading ? 'Wird hochgeladen…' : 'Kamera'}</span>
                      <input type="file" accept="image/*" capture="environment" className="sr-only" disabled={imageUploading} onChange={async e => { const f = e.target.files?.[0]; if (!f || !horse.ownerId) return; setImageUploading(true); try { const url = await uploadHorseImage(f, horse.ownerId, horse.id); setEditedHorse(prev => ({ ...prev, image: url })); } catch (_) {} finally { setImageUploading(false); e.target.value = ''; } }} />
                    </label>
                    <label className="flex items-center gap-2 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-100 text-sm font-medium text-slate-700">
          ...
          <form onSubmit={handleEntrySubmit} className="modal-form bg-white rounded-2xl sm:rounded-[2.5rem] p-3 sm:p-10 max-w-lg w-full max-w-[min(100vw-1rem,32rem)] shadow-2xl animate-in zoom-in-95 duration-200 space-y-3 sm:space-y-6 max-h-[calc(100dvh-2rem)] overflow-y-auto custom-scrollbar my-2 sm:my-auto modal-content">
            ...
            {showVaccModal && !editingItem && (
              <details className="group">
                <summary className="list-none cursor-pointer inline-flex items-center gap-2 text-slate-500 hover:text-indigo-600">
                  <span className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 group-open:bg-indigo-100 group-open:text-indigo-600">i</span>
